...
public class GuideCommandController {
  ...
  /**
   * Constructor for GuideCommandController.
   *
   * @param guideCommandService The guide command service.
   * @param guideQueryService   The guide query service.
   */
  public GuideCommandController(GuideCommandService guideCommandService,
  ...
  public ResponseEntity<GuideResource> createGuide(@Valid @RequestBody
                                                   CreateGuideResource resource) {
    var createGuideCommand = CreateGuideCommandFromResourceAssembler
        .toCommandFromResource(resource);
    var guideId = guideCommandService.handle(createGuideCommand);
    if (guideId == null || guideId == 0L) {
